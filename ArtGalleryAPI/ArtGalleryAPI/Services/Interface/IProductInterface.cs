...
namespace ArtGalleryAPI.Services.Interface
{
    public interface IProductInterface
    {
        ...
        Task<IEnumerable<Product>> GetAllProductsForAdminAsync(int pageNumber, int pageSize, string? query = null, string? sortBy = null, string? sortOrder = null, Guid? categoryId = null);

        Task<Product>? GetProductByIdAsync(Guid productId);
        Task<int>? GetProductsCountAsync(string? query, Guid? categoryId);
        Task<int>? GetProductsCountForAdminAsync(string? query, Guid? categoryId);
